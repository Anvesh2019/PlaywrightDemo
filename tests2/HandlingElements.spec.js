...
(0, test_1.test)('Handle text box', async () => {
    const browser = await test_2.chromium.launch({ headless: false, channel: 'chrome' });
    const page = await browser.newPage();
    await page.goto('https://www.google.com');
    const txtSrch = page.locator("[name='q']");
    txtSrch.fill("India");
    const length = await txtSrch.getAttribute("maxlength");
    console.log("max length is:" + length);
    await (0, test_1.expect)(txtSrch).toHaveValue("India");
    const txtValue = await txtSrch.inputValue();
    await (0, test_1.expect)(txtValue).toContain("India");
});
(0, test_1.test)('Handling Dropdown', async () => {
    const browser = await test_2.chromium.launch({ headless: false, channel: 'chrome' });
    const page = await browser.newPage();
...
(0, test_1.test)('Handling Listbox', async () => {
    const browser = await test_2.chromium.launch({ headless: false, channel: 'chrome' });
    const page = await browser.newPage();
    ...
    */
    ...
    const fruitsDD1 = await page.locator('select#fruits');
...
(0, test_1.test)('Iterate through all options from listbox', async () => {
    const browser = await test_2.chromium.launch({ headless: false, channel: 'chrome' });
    const page = await browser.newPage();
    await page.goto('https://output.jsbin.com/osebed/2');
    const countryDD = 'select#fruits';
    const fruitsDD = await page.locator('select#fruits');
